			       ...
			       int xs, int ys, int zs, int N)
{
  ...
  for (int i=0; i<4; i++)
	...
	val += abc[index] * base_addr[offset];
	index++;
      }
  vals[offset] = val;
}
...
test_cuda()
{
  ...
  cudaMalloc((void**)&abc_d, 64*sizeof(float));
  for (int i=0; i<64; i++)
    abc[i] = drand48();
  cudaMemcpy(abc_d, abc, 64*sizeof(float), cudaMemcpyHostToDevice);
  ...
  double time = (double)(end-start)/(double)(CLOCKS_PER_SEC*10000*N);
