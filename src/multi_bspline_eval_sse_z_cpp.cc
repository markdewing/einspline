...
eval_multi_UBspline_1d_z (multi_UBspline_1d_z *spline,
			  double x,
			  std::complex<double>* restrict vals)
{
  ...
  std::complex<double>* restrict coefs = spline->coefs;
  ...
  for (int i=0; i<4; i++) {
    std::complex<double>* restrict coefs = spline->coefs + ((ix+i)*xs);
    for (int n=0; n<spline->num_splines; n++) 
      vals[n]  +=   a[i] * coefs[n];
  }
}
...
eval_multi_UBspline_1d_z_vg (multi_UBspline_1d_z *spline,
			     double x,
			     std::complex<double>* restrict vals,
			     std::complex<double>* restrict grads)
{
  x -= spline->x_grid.start;
  double ux = x*spline->x_grid.delta_inv;
  double ipartx, tx;
...
eval_multi_UBspline_1d_z_vgl (multi_UBspline_1d_z *spline,
			      ...
			      std::complex<double>* restrict vals,
			      std::complex<double>* restrict grads,
			      std::complex<double>* restrict lapl)	  
{
  x -= spline->x_grid.start;
  double ux = x*spline->x_grid.delta_inv;
  double ipartx, tx;
...
eval_multi_UBspline_1d_z_vgh (multi_UBspline_1d_z *spline,
			      ...
			      std::complex<double>* restrict vals,
			      std::complex<double>* restrict grads,
			      std::complex<double>* restrict hess)
{
...
/************************************************************/
/* 2D double-precision, std::complex evaulation functions        */
/************************************************************/
void
...
//   std::complex<double>* restrict coefs = spline->coefs + ix*xs + iy*ys + iz;
//   std::complex<double>* restrict next_coefs = coefs + ss;
